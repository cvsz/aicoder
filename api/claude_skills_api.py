...
currently reimplement this by hand (pandas/openpyxl, python-pptx) with a
denylist-guarded exec loop — a real, working, already-tested alternative,
just not Anthropic's own maintained implementation. Routing those modules
through this API instead is intentionally left as a separate follow-up
(--excel-native / --pptx-native), not bundled into this base client, so
the existing hand-rolled path keeps working as the no-Skills-access
fallback.
...
call_with_skills_turn() / build_user_content() / extract_output_file_ids()
below are the pieces claude_excel.py and claude_powerpoint.py use to run
their chat loops entirely server-side through this API (xlsx/pptx Skills
in a code-execution container) instead of the hand-rolled pandas/openpyxl
