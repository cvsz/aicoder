...
`claude_fable5.py`'s `Fable5Client` already had a client-side manual
...
appeared nowhere in the codebase. That was wrong: `claude_tools.py`
...
real gap, found while implementing this, was narrower: `claude_code.py`'s
`--code-agent` loop never called it.

```bash
zai-coder --code-agent -p "long multi-file refactor" \
...
```python
from claude_code import CodeAgent, CodeSession
...
New module `claude_skills_api.py`. This is a *different* feature from
`claude_code.py`'s `SkillsRegistry`, which reads
`.claude/skills/<name>/SKILL.md` off the caller's local filesystem. The
...
```bash
zai-coder --skills-list
zai-coder --skills-info pptx
```
...
Routing `claude_excel.py` / `claude_powerpoint.py` through this instead of
...
New module `claude_admin_api.py`, combining both since they share the
same Admin API key requirement:

```bash
zai-coder --usage-report --admin-api-key sk-ant-admin-...
zai-coder --admin-list-keys --admin-api-key sk-ant-admin-...
zai-coder --admin-revoke-key key_abc123 --admin-api-key sk-ant-admin-...
```
...
`claude_cost_optimizer.py` is unchanged and still does local, estimate
