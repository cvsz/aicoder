...
class Coder:
    def __init__(self, api_key=None, model=None, temperature=0.3, max_tokens=4096,
                 ...
                 user_profile_id=None, effort=None):
        self.config = Config()
        ...
        # user_profile_id: sends anthropic-user-profile-id header with the
        # user-profiles beta header. Used to attribute API requests to a
        # specific user profile for multi-tenant applications (per
        # docs.anthropic.com, checked 2026-07-09).
        self.user_profile_id = user_profile_id
        # effort: output_config.effort level (low/medium/high/xhigh/max).
        # Controls model reasoning depth at the API level — separate from
        # thinking budgets in claude_thinking.py. Default is model-specific
        # (Opus 4.8 defaults to "high"; Sonnet 5 defaults to "high" on
        # API/Claude Code). Not sent unless explicitly set.
        self.effort = effort
    ...
    def generate(self, prompt, system=None, file_content=None, history=None):
        ...
        if self.fast_mode:
            payload["speed"] = "fast"
        # output_config.effort: API-level reasoning depth control (separate
        # from thinking budgets). Accepts low/medium/high/xhigh/max.
        if self.effort:
        ...
        # user-profile-id: attribute this request to a specific user profile
        # for multi-tenant billing/attribution (beta: user-profiles header).
        if self.user_profile_id:
            headers["anthropic-user-profile-id"] = self.user_profile_id
            existing_beta = headers.get("anthropic-beta", "")
            headers["anthropic-beta"] = (
                f"{existing_beta},user-profiles" if existing_beta else "user-profiles"
            )
